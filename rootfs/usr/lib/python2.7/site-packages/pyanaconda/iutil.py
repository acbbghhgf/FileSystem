...
from pyanaconda.constants import DRACUT_EJECT, DRACUT_SHUTDOWN_EJECT, TRANSLATIONS_UPDATE_DIR, UNSUPPORTED_HW
...
def dracut_eject(device):
    ...
    try:
        if not os.path.exists(DRACUT_EJECT):
            import shutil
            mkdirChain(os.path.dirname(DRACUT_EJECT))
            shutil.copyfile("/usr/bin/eject", DRACUT_EJECT)
            eintr_retry_call(os.chmod, DRACUT_EJECT, 0o755)
