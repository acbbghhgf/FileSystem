...
class PMONGRUB(GRUB2):
    _pmon_config_dir = "/boot"
    _pmon_config_file = "boot.cfg"
    ...
    def __init__(self):
        super(PMONGRUB, self).__init__()

    def config_path(self):
        """ Full path to pmon configuration file. """
        path = "%s/%s" % (self._pmon_config_dir, self._pmon_config_file)
        return os.path.normpath(iutil.getSysroot() + path)

    @property
    def boot_device_index(self):
        device = self.stage2_device
        index = "%d" % (device.partedPartition.number - 1,)
        return index
    ...
    def write_config(self):
        super(PMONGRUB, self).write_config()
        config = open(self.config_path(), "w")
        self.write_pmon_config_header(config)
        self.write_pmon_config_images(config)
        config.close()

    def write_pmon_config_header(self, config):
    ...
    def write_pmon_config_images(self, config):
        for image in self.images:
            args = Arguments()
            args.add("root=%s" % image.device.fstabSpec)
            if image.device.type == "btrfs subvolume":
                args.update(["rootflags=subvol=%s" % image.device.name])
            args.update(self.boot_args)
            stanza = ("title %(label)s, with Linux %(version)s\n"
                    "\tkernel (wd0,%(index)s)%(prefix)s/%(kernel)s\n"
                    "\tinitrd (wd0,%(index)s)%(prefix)s/%(initrd)s\n"
                    "\targs %(args)s\n\n"
                    % {"label": image.label, "version": image.version,
                        "index": self.boot_device_index, 
                        "prefix": self.boot_prefix,
                        "kernel": image.kernel, "args": args,
                        "initrd": image.initrd})
            config.write(stanza)
    def install(self, args=None):
        """ install the bootloader. Note: not implementated on mips"""
        pass
